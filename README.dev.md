# `openlab_vu` developer documentation
...
# install openlab_vu as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `openlab_vu`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d openlab_vu.XXXXXX)
git clone git@github.com:OSLA-project/openlab_vu .
...
Visit
[https://test.pypi.org/project/openlab_vu](https://test.pypi.org/project/openlab_vu)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d openlab_vu-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple openlab_vu
```
...
Don't forget to also make a [release on GitHub](https://github.com/OSLA-project/openlab_vu/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.